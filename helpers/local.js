...
const getFileContent = curry(
  ($workspace, packageUri) => console.log(packageUri)
    || $workspace.fs
      .readFile(packageUri)
      .then(Buffer.from)
      .then(JSON.parse)
...
const getLockFileContent = curry(($workspace, lockFileUri) => $workspace.fs
...
const getSubDependencies = ($workspace) => (currentDepsArray) => $workspace
